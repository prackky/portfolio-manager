...
function MfTransactionModal({ mf, onClose, onSave }) {
  const [transactions, setTransactions] = useState(mf.transactions || []); // Initialize with an empty array if null or undefined
  ...
  const handleSave = () => {
    const updatedmf = { ...mf, transactions };
    onSave(updatedmf);
    onClose();
  };
      ...
      <div className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow-lg w-1/2">
        ...
        {transactions.map((transaction, index) => (
          <div key={transaction.id} className="mb-4 grid grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-gray-600 dark:text-gray-200 mb-1">Units</label>
              <input
                type="number"
...
export default MfTransactionModal;
